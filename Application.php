...
class Application {
  ...
  public function __construct(bool $mvcDev = false, bool $errorReportingOverride = false, string|bool $projectNamespace = false) {
    ini_set('session.use_strict_mode', true);
    session_start();

    self::$isFrameworkDev = $mvcDev;
    self::$PATH_CORE = $mvcDev ? '' : $this->findPath('tolgaakyol\php-mvc\Application.php', 'php-mvc');
    self::$PATH_EXT = $mvcDev ? false : $this->findPath('index.php', 'src');
